...
void servo_tick(unsigned char cnt1, unsigned char cnt2)
{
	...
	switch(servo_state)
	{
		...
		case S_DOWN:
		(cnt1 >= 2 || cnt2 >= 2) ? (servo_state = S_UP) : (servo_state = S_DOWN) ; // if 2 motion detects then servo up
		break;
	...
	switch(servo_state)
	{
		...
		case S_RESET: // reset global count
		_count  = 0; 
		_motion = 0;
		break;
