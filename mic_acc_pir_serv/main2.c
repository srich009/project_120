...
void micro_tick(unsigned char tmpC);
...
void pir_tick(unsigned char tmpC)
{
	...
	switch(pir_state) // transitions
	{
		...
		// check if motion detected or not
		case P_WAIT:
		( !(tmpC & 0x01) ) ? (pir_state = P_ALERT) : (pir_state = P_WAIT); // added not sign
		break;
		
		case P_ALERT:
		( (tmpC & 0x01) ) ? (pir_state = P_WAIT) : (pir_state = P_ALERT); // removed not sign
		break;
...
void servo_tick(unsigned char cntPIR, unsigned char cntMIC)
{
	...
	switch(servo_state)
	{
		...
		if(cntPIR >= 1 || cntMIC >= 1) // ************ here error ??
		{
			servo_state = S_UP;
		}
		else
